...
```
cd GradingRahm
mkvirtualenv --no-site-packages GradingRahm
...
```
* * * * * ubuntu bash /home/ubuntu/apps/GradingRahm/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|GradingRahm|tweet|`location`||||
|GradingRahm|facebook|`location`||||
|GradingRahm|email|`location`||||
|GradingRahm|new-comment||||
|GradingRahm|open-share-discuss||||
|GradingRahm|close-share-discuss||||
|GradingRahm|summary-copied||||
|GradingRahm|featured-tweet-action|`action`||``tweet_url``|
|GradingRahm|featured-facebook-action|`action`||``post_url``|
